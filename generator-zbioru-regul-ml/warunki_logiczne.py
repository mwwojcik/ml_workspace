...
OPERATORY_POSTOPNIOWANE = podaj_dwuwymiarowy_iloczyn_kartezjanski(OPERATORY_POROWNANIA_STOPNIOWANE,
                                                                  OPERATORY_STOPNIOWANIA)
...
def utworz_warunki_czy_null():
    ...
    return pd.Series(tagger.taguj('xxx', 'porowanie_czy_null_OP') + ' ' + OPERATORY_CZY_NULL.apply(
        lambda x: tagger.taguj(x, 'operator_porownania_czy_null')))
...
def utworz_warunki_laczone(warunki_pojedyncze: pd.Series):
    ...
    wynik_mnozenia = narzedzia.wylicz_iloczyn_kartezjanski(
        [warunki_lewa_strona, operatory_logiczne_otagowane, warunki_prawa_strona], ['a', 'b', 'c'])
    # splaszczam i wybieram losowo n elementow gdzie n to dlugosc wektora WE
    return pd.Series(wynik_mnozenia['a'] + ' ' + wynik_mnozenia['b'] + ' ' + wynik_mnozenia[
        'c']).sample(n=int(len(warunki_pojedyncze)/3))


def utworz_warunki():
    warunki_pojedyncze_otagowane = pd.Series(
        tagger.taguj('xxx', 'OP_L') + ' ' + WYNIK.apply(lambda x: tagger.taguj(x, 'OPR_REL')) + ' ' + tagger.taguj(
            'xxx', 'OP_P'))
